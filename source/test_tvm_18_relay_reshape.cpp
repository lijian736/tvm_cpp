...
int main(int argc, char** argv) {
    ...
    if (!reshape) {
        std::cerr << "relay.op._make.reshape expression not found" << std::endl;
        return -1;
    }
    ...
    // the tensor type
    tvm::relay::TensorType tensor_type{{2, 3, 4, 5}, tvm::DataType{data_type}};

    // the var expression
    tvm::relay::Var var_expr = (*var_gen)(name, tensor_type, span);

    tvm::runtime::Array<tvm::Integer> new_shape({-3, -3});

    tvm::relay::Expr result = (*reshape)(var_expr, new_shape, false);
