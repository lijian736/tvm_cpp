...
int main(int argc, char** argv) {
    // Step 1. Get the divide packed function
    const tvm::runtime::PackedFunc* divide = tvm::runtime::Registry::Get("relay.op._make.divide");
    if (!divide) {
        std::cerr << "relay.op._make.divide expression not found" << std::endl;
        return -1;
    }
    ...
    tvm::relay::TensorType input_tensor_type1{{1, 4, 5}, tvm::DataType::Float(32)};
    // the input tensor type2, float32 and shape is {2, 3, 1, 1}
    tvm::relay::TensorType input_tensor_type2{{2, 3, 1, 1}, tvm::DataType::Float(32)};
    ...
    // Step 3. Compute the divide
    tvm::relay::Expr result_expr = (*divide)(input_var1, input_var2);
    ...
    if (type != nullptr) {
        ...
        std::cout << "Divide Output DataType: " << data_type << ", Shape: ";
        std::cout << "{";
