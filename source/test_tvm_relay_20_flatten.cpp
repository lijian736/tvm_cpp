...
int main(int argc, char** argv) {
    // Step 1. Get the batch_flatten packed function
    const tvm::runtime::PackedFunc* batch_flatten = tvm::runtime::Registry::Get("relay.op.nn._make.batch_flatten");
    if (!batch_flatten) {
        std::cerr << "relay.op.nn._make.batch_flatten expression not found" << std::endl;
        return -1;
    }
    ...
    // the input tensor type, float32 and shape is {3, 2048, 2, 2}
    tvm::relay::TensorType input_tensor_type{{3, 2048, 2, 2}, tvm::DataType{input_data_type}};

    // the input var expression
    tvm::relay::Var input_var = (*input_gen)(input_name, input_tensor_type, input_span);

    // Step 3. Compute the batch flatten
    tvm::relay::Expr result_expr = (*batch_flatten)(input_var);
    ...
    if (type != nullptr) {
        ...
        std::cout << "Batch Flatten Output DataType: " << data_type << ", Shape: ";
        std::cout << "{";
