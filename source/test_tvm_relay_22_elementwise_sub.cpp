...
int main(int argc, char** argv) {
    // Step 1. Get the subtract packed function
    const tvm::runtime::PackedFunc* subtract = tvm::runtime::Registry::Get("relay.op._make.subtract");
    if (!subtract) {
        std::cerr << "relay.op._make.subtract expression not found" << std::endl;
        return -1;
    }
    ...
    // Step 3. Compute the subtract
    tvm::relay::Expr result_expr = (*subtract)(input_var1, input_var2);
    ...
    if (type != nullptr) {
        ...
        std::cout << "Subtract Output DataType: " << data_type << ", Shape: ";
        std::cout << "{";
