...
int main(int argc, char** argv) {
    // Step 1. Get the sqrt packed function
    const tvm::runtime::PackedFunc* sqrt_relay = tvm::runtime::Registry::Get("relay.op._make.sqrt");
    if (!sqrt_relay) {
        std::cerr << "relay.op._make.sqrt expression not found" << std::endl;
        return -1;
    }
    ...
    // Step 3. Compute the sqrt
    tvm::relay::Expr result_expr = (*sqrt_relay)(input_var);
    ...
    if (type != nullptr) {
        ...
        std::cout << "Sqrt Output DataType: " << data_type << ", Shape: ";
        std::cout << "{";
