...
int main(int argc, char** argv) {
    // Step 1. Get the erf packed function
    const tvm::runtime::PackedFunc* erf_relay = tvm::runtime::Registry::Get("relay.op._make.erf");
    if (!erf_relay) {
        std::cerr << "relay.op._make.erf expression not found" << std::endl;
        return -1;
    }
    ...
    // Step 3. Compute the erf
    tvm::relay::Expr result_expr = (*erf_relay)(input_var);
    ...
    if (type != nullptr) {
        ...
        std::cout << "Erf Output DataType: " << data_type << ", Shape: ";
        std::cout << "{";
