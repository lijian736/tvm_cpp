...
int main(int argc, char** argv) {
    ...
    const tvm::runtime::PackedFunc* fold_const = tvm::runtime::Registry::Get("relay._transform.FoldConstantExpr");
    if (!fold_const) {
        std::cerr << "relay._transform.FoldConstantExpr expression not found" << std::endl;
        return -1;
    }
    ...
    tvm::runtime::NDArray initializer =
        tvm::runtime::NDArray::Empty({5, 5}, {DLDataTypeCode::kDLInt, 32, 1}, {DLDeviceType::kDLCPU, 0});
    // initialize the NDArray
    for (int i = 0; i < 25; ++i) {
        static_cast<int32_t*>(initializer->data)[i] = 1;
    }
    ...
    tvm::relay::Expr result = (*fold_const)(const_expr, tvm::IRModule(), false);
