...
int main(int argc, char** argv) {
    // Step 1. Get the softmax packed function
    const tvm::runtime::PackedFunc* softmax = tvm::runtime::Registry::Get("relay.op.nn._make.softmax");
    if (!softmax) {
        std::cerr << "relay.op.nn._make.softmax expression not found" << std::endl;
        return -1;
    }
    ...
    // the input tensor type, float32 and shape is {1, 32, 8, 20}
    tvm::relay::TensorType input_tensor_type{{1, 32, 8, 20}, tvm::DataType::Float(32)};
    ...
    int axis = 0;
    tvm::relay::Expr result_expr = (*softmax)(input_var, axis);
    ...
    if (type != nullptr) {
        ...
        std::cout << "Softmax Output DataType: " << data_type << ", Shape: ";
        std::cout << "{";
